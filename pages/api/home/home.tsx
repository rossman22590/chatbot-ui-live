...
const Home = ({ serverSideApiKeyIsSet, defaultModelId }: Props) => {
  ...
  const setDefaultSystemPromptForVendor = useCallback(
    (user: User, vendor: AiModel['vendor'], systemPromptId: string) => {
      ...
      const sectionId = vendor.toLowerCase();

      const savedSettings = getSavedSettings(user);

      for (const model of vendorModels) {
        const settingId = `${model}_default_system_prompt`;
  ...
  const autogenerateDefaultSystemPrompt = useCallback(
    (curSelectedConversation: Conversation) => {
      if (
        savedSettings &&
        settings &&
        conversations &&
        ...
        const model = curSelectedConversation.model;
        ...
        const newSettings = setDefaultSystemPromptForVendor(
          user,
          model.vendor,
          systemPrompt.id,
        );
    ...
    [
      ...
      dispatch,
      savedSettings,
      setDefaultSystemPromptForVendor,
      settings,
      systemPrompts,
      t,
      user,
  ...
  useEffect(() => {
    if (selectedConversation && !selectedConversation?.systemPrompt) {
      autogenerateDefaultSystemPrompt(selectedConversation);
    }
  }, [selectedConversation, autogenerateDefaultSystemPrompt]);
